...
# --- provenance ---
category: design
source: https://github.com/zeke/swiss-design-skill (swiss-design)
author: zeke
license: MIT
...
- Reference (do not auto-fetch): the Vercel Web Interface Guidelines, published at `vercel-labs/web-interface-guidelines`. If the user wants that audit, ask before fetching any remote file.

After applying Swiss styling, audit the result for accessibility, focus states, form behavior, animation safety, performance, and i18n using the principles above and any locally available guidelines. Treat findings as required fixes, not suggestions. Where the two systems overlap (typography niceties, dark mode, focus rings), the rules already inlined in this SKILL.md take precedence.

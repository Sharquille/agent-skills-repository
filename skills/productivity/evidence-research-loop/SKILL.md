...
description: "Answer a specific factual or technical question through a disk-backed, citation-audited research pipeline: decompose, find primary sources, extract evidence, identify conflicts, synthesize, audit citations. Built on agent-orchestra: the conductor thinks while heavy source reading is delegated to the Codex flagship and OpenCode lanes to preserve Claude usage. Use when the user wants an evidence-first answer, primary-source research, quote-level citations, adjudication of conflicting sources, a defensible research memo, or says 'research this properly / with sources / with citations'. When an obsidian-study-loop vault session is active and relevant, run session-integrated as a research dive. Do not trigger for full academic literature reviews (use literature-review), study-gap query planning (use study-research-queries), or quick lookups a single authoritative source settles."
# --- provenance ---

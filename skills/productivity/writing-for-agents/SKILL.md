...
description: "Writing documents an agent consumes — skills, AGENTS.md / CLAUDE.md / GEMINI.md, and any doc reached by a pointer. Covers context pointers and trigger wording, the information hierarchy and progressive disclosure, completion criteria, when to split a document, leading words, and pruning for duplication, no-ops, and sediment. Use when authoring, importing, enhancing, or trimming a skill, when a skill fires too often or not enough, or when editing global agent instruction files. Do not trigger for writing prose for humans (use humanizer) or for auditing an untrusted skill's safety (use vet-skill)."
# --- provenance ---
...
modified-by: Sharquille Andrew (description expanded to this repo's trigger/anti-trigger convention; local skill cross-references appended)
---
...
- When a global pointer names an authority, point to it without copying its rules. An always-loaded pointer should state the default action and the path to the full standard, not become a second policy source.
...
This skill governs a document's _structure_: what sits where on the hierarchy, what fires it, where it splits. Three neighbours own the rest, so keep each meaning in one of them:

- **`anti-slop-standard`** — sentence-level quality of whatever you write, agent-facing or not. Structure here, prose there.
- **`vet-skill`** — a security audit of untrusted skill files before installing. Run it before this one on anything imported.
- **`enhance-skill`** and **`name-skill`** — the workflows that apply this reference to an existing skill and settle what it is called.

Skills in this repo carry a provenance block in their frontmatter (`category`, `source`, `author`, `license`, `retrieved`, and `modified-by` when adapted). It is metadata, not a pointer, and costs nothing at load time — see `skills/_template/SKILL.md` and `REGISTRY.md`.

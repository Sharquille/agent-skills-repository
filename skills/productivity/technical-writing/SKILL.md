...
modified-by: "Sharquille Andrew — imported verbatim except the Global English em-dash line, softened to match this repo's house style and the local unslop rule 13. Underlying sources (diataxis.fr, Google developer style, ASD-STE100, Kohl's Global English) carry their own terms; this skill is a summary of their principles, not a reproduction."
---
...
- Use periods, not semicolons. Where an em dash is carrying a whole clause, a new sentence usually reads better; keep the dash for a genuine aside (see `unslop` rule 13).
...
- `unslop` is the default prose pass. This skill owns document mode, structure, sentence load, and ambiguity; it does not authorize stylistic rewriting that changes technical meaning.

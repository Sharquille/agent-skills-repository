...
description: "Design experiments to test assumptions for an existing product — prototypes, A/B tests, spikes, and other low-effort validation methods. Use when validating assumptions, testing feature ideas cheaply, or planning product experiments. Do not trigger for new or greenfield products — use brainstorm-experiments-new instead."
# --- provenance (added on import; ignored by skill loader) ---
category: productivity
source: https://github.com/phuryn/pm-skills/tree/main/pm-product-discovery/skills/brainstorm-experiments-existing
...
If `$ARGUMENTS` is empty, ask the user: "Which existing product or feature are we designing experiments for?" before proceeding.

If the user provides files (PRDs, assumption lists, designs), read them first.

> **Sibling skill:** for a new or greenfield product, use `brainstorm-experiments-new` instead.

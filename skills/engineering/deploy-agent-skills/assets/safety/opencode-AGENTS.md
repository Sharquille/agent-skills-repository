...
OpenCode lanes: `--lane code` = Kimi K2.7 Code (technical/config/security), `--lane reasoning` = MiniMax M3 (deep reasoning, high effort), `--lane context` = DeepSeek V4 Flash (cheap ~1M-context sweeps), `--lane prose` = MiMo v2.5 Pro (writing). Run lanes sequentially. The Codex flagship lane (`gpt-5.6-sol`, default effort medium, high/xhigh per call for hard tasks, never max/ultra) is the main implementor; only when it rate-limits, times out, or is down, fall back to OpenCode (consult -> `--lane reasoning` or `--lane context`; review -> sealed diff to `--lane code`; implement -> `opencode-implement.sh --allow-write`, file edits only, no shell) instead of pulling bulk work into Claude.

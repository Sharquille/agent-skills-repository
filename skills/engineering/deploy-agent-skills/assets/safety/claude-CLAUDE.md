...
OpenCode lanes: `--lane code` = Kimi K2.7 Code (technical/config/security checks), `--lane reasoning` = MiniMax M3 (deep reasoning at high effort), `--lane context` = DeepSeek V4 Flash (cheap ~1M-context sweeps of logs/diffs/repos), `--lane prose` = MiMo v2.5 Pro (writing/readability). Run lanes sequentially. The Codex flagship lane (`gpt-5.6-sol`, default effort medium, high/xhigh per call for hard tasks, never max/ultra) is the main implementor; only when Codex rate-limits, times out, or is down, step down to OpenCode instead of pulling bulk work into Claude: consult -> `--lane reasoning` (hard thinking) or `--lane context` (input-heavy sweeps); review -> local `git diff` sealed to `--lane code`; implement -> `opencode-implement.sh --allow-write --scope <path>` (file edits only, no shell; the conductor runs tests and owns git).

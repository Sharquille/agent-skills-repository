...
description: "Review Cisco IOS / IOS-XE configuration safely. Use when reviewing IOS or IOS-XE snippets before a change, choosing read-only show commands for troubleshooting, checking ACL wildcard masks and interface direction, explaining config modes, or building a change-window checklist and collecting evidence from a router or switch without making an incident worse."
# --- provenance ---
category: engineering
source: skillrepo.dev (Affaan Mustafa) — pasted by user
